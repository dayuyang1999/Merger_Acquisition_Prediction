    ...
    for epoch in range(15):
        ...
        for i, batch in enumerate(tqdm(loader)):
            ...
            if False:#torch.cuda.is_available():
                ...
                arr_c = arr_c.to(device)
                arr_delta_time = arr_delta_time.to(device)
                for i, ele in enumerate(event_data):
                    event_data[i] = ele.to(device)
                for i, ele in enumerate(non_event_data):
                    non_event_data[i] = ele.to(device)
                estimate_length = estimate_length.to(device)
                for i in range(0, 4):
                    if i == 0:
                        for key, value in choice_data_dict[i].items(): # choice_data_dict[i] = dict_idx
                            for j, ele in enumerate(value):
                                choice_data_dict[i][key][j] = ele.to(device)
                    else:
